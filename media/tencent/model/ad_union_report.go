...
// AdUnionReportsGetReq 获取联盟广告位报表请求
type AdUnionReportsGetReq struct {
	GlobalReq
	AccountID int64                   `json:"account_id"`          // 广告主帐号id (必填)
	Filtering *AdUnionReportFiltering `json:"filtering,omitempty"` // 过滤条件
	DateRange *AdUnionReportDateRange `json:"date_range"`          // 日期范围 (必填)
	GroupBy   []string                `json:"group_by,omitempty"`  // 聚合参数
	OrderBy   []*AdUnionReportOrderBy `json:"order_by,omitempty"`  // 排序字段
...
// AdUnionReportFiltering 过滤条件
type AdUnionReportFiltering struct {
	UnionPositionID []int64 `json:"union_position_id,omitempty"` // 联盟广告位id列表，1-20个
}

// AdUnionReportDateRange 日期范围
type AdUnionReportDateRange struct {
	StartDate string `json:"start_date"` // 开始日期，格式：YYYY-MM-DD (必填)
	EndDate   string `json:"end_date"`   // 结束日期，格式：YYYY-MM-DD (必填)
}

// AdUnionReportOrderBy 排序字段
type AdUnionReportOrderBy struct {
	SortField string `json:"sort_field"` // 排序字段 (必填)
	SortType  string `json:"sort_type"`  // 排序方式 (必填)
}

// 联盟广告位报表 group_by 可选值
const (
	AdUnionGroupByUnionPositionID  = "union_position_id"
	AdUnionGroupByDate             = "date"
	AdUnionGroupByIndustryParentID = "industry_parent_id"
	AdUnionGroupByPlacementType    = "placement_type"
)

// 联盟广告位报表限制常量
const (
	AdUnionReportMinFilteringCount = 1
	AdUnionReportMaxFilteringCount = 20
	AdUnionReportMinGroupByCount   = 1
	AdUnionReportMaxGroupByCount   = 5
	AdUnionReportMaxGroupByLength  = 255
	AdUnionReportMinOrderByCount   = 1
	AdUnionReportMaxOrderByCount   = 2
	AdUnionReportMinPage           = 1
	AdUnionReportMaxPage           = 99999
	AdUnionReportMinPageSize       = 1
	AdUnionReportMaxPageSize       = 100
	AdUnionReportDefaultPage       = 1
	AdUnionReportDefaultPageSize   = 10
	AdUnionReportMinFieldsCount    = 1
	AdUnionReportMaxFieldsCount    = 1024
	AdUnionReportMinFieldLength    = 1
	AdUnionReportMaxFieldLength    = 64
	AdUnionReportDateLength        = 10
)

func (p *AdUnionReportsGetReq) Format() {
	p.GlobalReq.Format()
	if p.Page <= 0 {
		p.Page = AdUnionReportDefaultPage
	}
	if p.PageSize <= 0 {
		p.PageSize = AdUnionReportDefaultPageSize
	}
}

func (p *AdUnionReportsGetReq) Validate() error {
	// 验证全局参数
	...
	// 验证filtering
	if p.Filtering != nil {
		if len(p.Filtering.UnionPositionID) > 0 {
			if len(p.Filtering.UnionPositionID) < AdUnionReportMinFilteringCount || len(p.Filtering.UnionPositionID) > AdUnionReportMaxFilteringCount {
				return errors.New("filtering.union_position_id数组长度必须在1-20之间")
			}
		}
	}
	...
	// 验证group_by
	if len(p.GroupBy) > 0 {
		if len(p.GroupBy) < AdUnionReportMinGroupByCount || len(p.GroupBy) > AdUnionReportMaxGroupByCount {
			return errors.New("group_by数组长度必须在1-5之间")
		}
		for _, g := range p.GroupBy {
			if len(g) > AdUnionReportMaxGroupByLength {
	...
	// 验证order_by
	if len(p.OrderBy) > 0 {
		if len(p.OrderBy) < AdUnionReportMinOrderByCount || len(p.OrderBy) > AdUnionReportMaxOrderByCount {
	...
	// 验证page
	if p.Page < AdUnionReportMinPage || p.Page > AdUnionReportMaxPage {
		return errors.New("page必须在1-99999之间")
	}

	// 验证page_size
	if p.PageSize < AdUnionReportMinPageSize || p.PageSize > AdUnionReportMaxPageSize {
		return errors.New("page_size必须在1-100之间")
	}

	// 验证fields (必填)
	if len(p.Fields) < AdUnionReportMinFieldsCount || len(p.Fields) > AdUnionReportMaxFieldsCount {
		return errors.New("fields数组长度必须在1-1024之间")
	}
	for _, f := range p.Fields {
		if len(f) < AdUnionReportMinFieldLength || len(f) > AdUnionReportMaxFieldLength {
...
// AdUnionReportsGetResp 获取联盟广告位报表响应
type AdUnionReportsGetResp struct {
	List     []*AdUnionReportListItem `json:"list,omitempty"`
	PageInfo *PageInfo                `json:"page_info,omitempty"`
}

// AdUnionReportListItem 联盟广告位报表列表项
type AdUnionReportListItem struct {
