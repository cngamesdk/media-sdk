...
func TestAppOnline(t *testing.T) {
	ctx := context.Background()
	req := &kuaishouModel.AppOnlineReq{}
	req.AccessToken = "your_access_token"
	req.AdvertiserId = 900015366
	req.PackageIds = []int64{1099611634749}
	adapter := NewKuaishouAdapter(config.DefaultConfig())
	resp, err := adapter.AppOnline(ctx, req)
	if err != nil {
		t.Fatal(err)
	}
	fmt.Printf("result=%v\n", resp.Result)
...
func TestAppOnlineValidation(t *testing.T) {
	ctx := context.Background()
	adapter := NewKuaishouAdapter(config.DefaultConfig())
	...
	req.AdvertiserId = 900015366
	req.PackageIds = []int64{1099611634749}
	_, err := adapter.AppOnline(ctx, req)
	if err == nil {
	...
	req3 := &kuaishouModel.AppOnlineReq{}
	req3.AccessToken = "your_access_token"
	req3.AdvertiserId = 900015366
	_, err3 := adapter.AppOnline(ctx, req3)
	if err3 == nil {
