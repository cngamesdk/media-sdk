...
// AppApkUpload 上传APK文件
// https://ad.e.kuaishou.com/rest/openapi/gw/dsp/appcenter/upload/apk
func (a *KuaishouAdapter) AppApkUpload(ctx context.Context, req *kuaishouModel.AppApkUploadReq) (resp *kuaishouModel.AppApkUploadResp, err error) {
	req.Format()
	...
	respBytes, requestErr := a.Media.Client.Request(ctx, http.MethodPost, kuaishouModel.AdUrl+"/rest/openapi/gw/dsp/appcenter/upload/apk", body, headers)
	if requestErr != nil {
		err = requestErr
		return
	}
	...
	var result kuaishouModel.AppApkUploadResp
