...
// ProgramCreativeReportReq 程序化创意数据请求（包含程序化2.0+智能创意，不包括省心投物料）
type ProgramCreativeReportReq struct {
	accessTokenReq
	AdvertiserId        int64    `json:"advertiser_id"`                  // 广告主id，必填
	UnitType            int      `json:"unit_type,omitempty"`            // 广告组创意类型：7=程序化2.0创意，10=智能创意，不传返回全部
...
func (receiver *ProgramCreativeReportReq) Format() {
	receiver.accessTokenReq.Format()
}

func (receiver *ProgramCreativeReportReq) Validate() (err error) {
...
// ProgramCreativeReportDetail 程序化创意数据明细
type ProgramCreativeReportDetail struct {
	// 日期/时间
	...
	PhotoUrl      string `json:"photo_url"`       // 视频链接
	PhotoMd5      string `json:"photo_md5"`       // 视频MD5
	CoverUrl      string `json:"cover_url"`       // 封面图链接
	ImageToken    string `json:"image_token"`     // 封面图ID
	Description   string `json:"description"`     // 广告语
	PicUrl        string `json:"pic_url"`         // 图片链接
	PicImageToken string `json:"pic_image_token"` // 图片token
	PicId         int64  `json:"pic_id"`          // 图片库图片ID
...
// ProgramCreativeReportResp 程序化创意数据响应数据（仅data部分）
type ProgramCreativeReportResp struct {
	TotalCount int64                         `json:"total_count"` // 数据的总行数
	Details    []ProgramCreativeReportDetail `json:"details"`     // 数据明细信息
}
