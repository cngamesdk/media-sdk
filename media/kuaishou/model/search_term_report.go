...
// SearchTermReportReq 搜索词报表请求
type SearchTermReportReq struct {
	accessTokenReq
	...
	Query               []string `json:"query,omitempty"`                // 搜索词列表
	SelectedColumns     []string `json:"selected_columns,omitempty"`     // 选择列
}

func (receiver *SearchTermReportReq) Format() {
	receiver.accessTokenReq.Format()
}

func (receiver *SearchTermReportReq) Validate() (err error) {
...
// SearchTermReportDetail 搜索词报表数据明细
type SearchTermReportDetail struct {
	// 维度标识
	...
	Query         string `json:"query"`          // 搜索词文本
	TriggerType   int    `json:"trigger_type"`   // 触发类型
...
// SearchTermReportResp 搜索词报表响应数据（仅data部分）
type SearchTermReportResp struct {
	TotalCount int64                    `json:"total_count"` // 数据的总行数
	Details    []SearchTermReportDetail `json:"details"`     // 数据明细信息
}
