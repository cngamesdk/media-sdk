...
// AudienceReportReq 人群分析报表请求
type AudienceReportReq struct {
	accessTokenReq
	AdvertiserId int64   `json:"advertiser_id"`          // 广告主ID，必填
	StartDate    string  `json:"start_date"`             // 开始日期，格式 YYYY-MM-DD，只支持查询从今日起一年以内日期，必填
	EndDate      string  `json:"end_date"`               // 结束日期，格式 YYYY-MM-DD，只支持查询昨天之前的数据，必填
	...
	ReportDims   string  `json:"report_dims"`            // 人群报表类型（必填）：province=省级地域分布 city=地级地域分布 gender=性别分布 ageSegment=年龄分布 clientId=系统分布
	Page         int     `json:"page,omitempty"`         // 请求页码，默认为1
	PageSize     int     `json:"page_size,omitempty"`    // 每页行数，默认为100，最大支持2000
}

func (receiver *AudienceReportReq) Format() {
	receiver.accessTokenReq.Format()
}

func (receiver *AudienceReportReq) Validate() (err error) {
	...
	if len(receiver.ReportDims) <= 0 {
		err = errors.New("report_dims is empty")
		return
	}
	return
}

// AudienceReportDetail 人群分析报表数据明细
type AudienceReportDetail struct {
	// 日期/时间
	StatDate string `json:"stat_date"` // 数据日期，格式：YYYY-MM-DD
	StatHour int64  `json:"stat_hour"` // 数据小时

	// 人群维度字段（根据report_dims返回对应字段）
	Province   string `json:"province"`    // 省份（report_dims=province时返回）
	City       string `json:"city"`        // 城市（report_dims=city时返回）
	Gender     string `json:"gender"`      // 性别（report_dims=gender时返回）
	AgeSegment string `json:"age_segment"` // 年龄（report_dims=ageSegment时返回）
	Client     string `json:"client"`      // 系统（report_dims=clientId时返回）
	...
	EventEffectiveCustomerAcquisitionCnt   int64   `json:"event_effective_customer_acquisition_cnt"`   // 有效获客数
	EventEffectiveCustomerAcquisitionCost  float64 `json:"event_effective_customer_acquisition_cost"`  // 有效获客成本
	EventEffectiveCustomerAcquisitionRatio float64 `json:"event_effective_customer_acquisition_ratio"` // 有效获客率
...
// AudienceReportResp 人群分析报表响应数据（仅data部分）
type AudienceReportResp struct {
	TotalCount int64                  `json:"total_count"` // 数据的总行数
	Details    []AudienceReportDetail `json:"details"`     // 数据明细信息
}
