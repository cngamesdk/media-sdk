...
// UnitReportReq 广告组数据请求（不包括省心投物料）
type UnitReportReq struct {
	accessTokenReq
...
func (receiver *UnitReportReq) Format() {
	receiver.accessTokenReq.Format()
}

func (receiver *UnitReportReq) Validate() (err error) {
...
type UnitReportDetail struct {
	...
	UnitSource   int    `json:"unit_source"`   // 0=非托管 1=托管
...
// UnitReportResp 广告组数据响应数据（仅data部分）
type UnitReportResp struct {
	TotalCount int64              `json:"total_count"` // 数据的总行数
	Details    []UnitReportDetail `json:"details"`     // 数据明细信息
}
