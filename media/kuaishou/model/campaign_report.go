...
type CampaignReportReq struct {
	...
	CampaignType        int      `json:"campaign_type,omitempty"`        // 广告计划类型：1=作品推广 2=提升应用安装 3=获取电商下单 4=推广品牌活动 5=收集销售线索 6=保量广告 7=提高应用活跃
...
func (receiver *CampaignReportReq) Format() {
	receiver.accessTokenReq.Format()
}

func (receiver *CampaignReportReq) Validate() (err error) {
...
type CampaignReportDetail struct {
	...
	// 回流预估
	BackflowCvLower      int     `json:"backflow_cv_lower"`      // 预估总转化数下限
	BackflowCvUpper      int     `json:"backflow_cv_upper"`      // 预估总转化数上限
	ConversionCostLower  float64 `json:"conversion_cost_lower"`  // 预估转化成本下限
	ConversionCostUpper  float64 `json:"conversion_cost_upper"`  // 预估转化成本上限
	BackflowPayment      float64 `json:"backflow_payment"`       // 预估首日付费金额
	BackflowRoi          float64 `json:"backflow_roi"`           // 预估首日ROI
	BackflowSevenPayment float64 `json:"backflow_seven_payment"` // 预估七日付费金额
	BackflowSevenRoi     float64 `json:"backflow_seven_roi"`     // 预估七日ROI

	// MCB（最大转化出价）表现
	McbLeftTimePerfCpaBid   float64 `json:"mcb_left_time_perf_cpa_bid"`   // 剩余时段转化目标成本
	McbLeftTimePerfRoiRatio float64 `json:"mcb_left_time_perf_roi_ratio"` // 剩余时段ROI
	McbAllDayPerfCpaBid     float64 `json:"mcb_all_day_perf_cpa_bid"`     // 全天转化目标成本
	McbAllDayPerfRoiRatio   float64 `json:"mcb_all_day_perf_roi_ratio"`   // 全天ROI
...
// CampaignReportResp 广告计划数据响应数据（仅data部分）
type CampaignReportResp struct {
	TotalCount int64                  `json:"total_count"` // 数据的总行数
	Details    []CampaignReportDetail `json:"details"`     // 数据明细信息
}
